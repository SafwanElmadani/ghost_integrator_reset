...
// Generated 09-Oct-2020 19:37:22
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"ghost_integrator\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(ghost_integrator_M, (NULL));
        ghost_integrator_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(ghost_integrator_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(ghost_integrator_M)
                == (NULL)) || rtmGetStopRequested(ghost_integrator_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(ghost_integrator_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
